...
function daytonaDesktopAgentsMd() {
  return [
    ...
    "",
    "## GUI and Browser Work",
    "",
    "Cloudcode may provide a Daytona desktop for GUI/browser work through the `cloudcode_desktop` MCP tools: the `browser_*` tools drive the headed desktop browser through Playwright with real, trusted user input (prefer them for anything inside a web page); the `desktop_*` tools control the desktop outside the browser.",
    "After UI-facing code changes, verify the change by driving the app with the `browser_*` tools.",
    "Before your first `browser_*` or `desktop_*` call in a run, read the `cloudcode-computer-use` skill and follow it exactly — it holds the verification rules (end-to-end flows, no cheating or faked state), the dev-server workflow, and the efficient tool sequences. Do not assume any dev server is running; the skill covers checking and starting one.",
    "Never launch `chromium`, `chromium-browser`, `google-chrome`, `google-chrome-stable`, `firefox`, `x-www-browser`, or `xdg-open` directly, and never drive a browser with your own Playwright or scripts; the Cloudcode MCP tools manage Cloudcode Browser.",
    "",
    "## Deterministic UI Tests (opt-in only)",
    "",
    "The `cloudcode_ui_tests` MCP runs deterministic, recorded Playwright specs from `.cloudcode/tests`. Strictly opt-in: write or run them only when the user explicitly asks for a deterministic, recorded test of a specific flow — never as the default way to verify a change, and never as a side effect of another task. If a change clearly deserves one, finish the task first, then offer to add it and wait for the user to confirm.",
    "Before writing or running any `.cloudcode/tests` spec, read the `cloudcode-ui-tests` skill and follow it exactly — it holds the required spec template, the runner's hard rules, and the run workflow.",
    "",
    "## Showing Changes",
    "",
    "Cloudcode captures the diff of your changes automatically after every turn and shows it to the user in the app. Never run `git diff` (or similar) at the end of a run just to display, review, or summarize what you changed — finish with your text summary instead. Run diff commands only when you need the output yourself to keep working.",
    "",
    "## GitHub Operations",
    "",
...
function desktopCodexConfig(
  ...
  return [
    // Cloudcode installs SKILL.md playbooks into $CODEX_HOME/skills; skill
    // discovery is gated behind this feature flag in Codex.
    "[features]",
    "skills = true",
    "",
